...
| `codex` | `codex-companion.mjs` | **Gerüst** — Kommandobau getestet, Aufrufweg nicht live geprüft |
| `agy` | Antigravity/Gemini | **Gerüst** — Rückgabe zwingend über Datei (kein stdout) |
| `kimi` | Kimi Code CLI | **Gerüst** — `-p` nicht mit `--yolo`/`--auto` kombinierbar |
