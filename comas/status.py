"""Der Statusschreiber — **einziger** Schreiber von ``comas.<jobid>.json``.

Vorlage: ``_control-center/_agentjobs/comas_status.py``. Die Schluessel sind
...
Das Modul bleibt ausserdem als Skript aufrufbar und argv-kompatibel zur Vorlage::
    ...
    python -m comas.status start  <comasfile> <jobid> <model> <jobfile> <resultfile>
    python -m comas.status finish <comasfile> <exitcode>
"""
...
def now() -> str:
    """Zeitstempel im Format der Vorlage (``comas_status.py:18-19``)."""
...
class StatusWriter:
    ...
    def finish(
        ...
        Vorlage (``comas_status.py:43-47``). Der Zustand darf nie ganz fehlen.
        """
...
def main(argv: list[str] | None = None) -> int:
    """Argv-kompatibler Einstieg zur Vorlage ``comas_status.py``."""
    argv = sys.argv if argv is None else argv
    if len(argv) < 3:
        print("usage: comas.status start|finish <comasfile> ...", file=sys.stderr)
        return 2
    ...
    if mode == "start":
        if len(argv) < 7:
            print(
                "usage: comas.status start <comasfile> <jobid> <model> "
                "<jobfile> <resultfile>",
                file=sys.stderr,
            )
            return 2
        ...
        if len(argv) < 4:
            print("usage: comas.status finish <comasfile> <exitcode>", file=sys.stderr)
            return 2
